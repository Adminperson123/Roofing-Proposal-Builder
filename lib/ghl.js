...
/**
 * Send an Email to a GHL contact. Requires contactId (the contact must exist in GHL).
 * GHL renders `html`; if only plain `message` is given we wrap it, escaping the
 * text first so customer-facing copy can't break or inject markup.
 */
export async function sendEmail({ contactId, subject, html, message }) {
  if (!contactId) return { ok: false, error: 'contactId required for email' }
  if (!subject)   return { ok: false, error: 'subject required for email' }
  const bodyHtml = html || (message ? `<p>${escapeHtml(message).replace(/\n/g, '<br>')}</p>` : null)
  if (!bodyHtml) return { ok: false, error: 'email body (html or message) required' }
